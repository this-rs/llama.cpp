...
llama_kv_cache::llama_kv_cache(
    ...
    for (uint32_t il = 0; il < n_layer; il++) {
        ...
        // [obrain] TurboQuant dropped in the 2026-07 upstream merge (unused in
        // production, recoverable on the `thetom` remote). No layer-adaptive
        // type selection or head_dim padding — use the configured types and
        // effective GQA embeddings directly.
        const bool has_k = true;
        const bool has_v = !is_mla;

        const ggml_type layer_type_k = type_k;
        const ggml_type layer_type_v = type_v;
        const uint32_t  n_embd_k_gqa_eff = n_embd_k_gqa;
        const uint32_t  n_embd_v_gqa_eff = n_embd_v_gqa;
...
ggml_tensor * llama_kv_cache::get_k(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    ...
    const bool k_is_turbo = false; // [obrain] TurboQuant dropped in 2026-07 merge
    if (k_is_turbo) {
...
ggml_tensor * llama_kv_cache::get_v(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    ...
    const bool v_is_turbo = false; // [obrain] TurboQuant dropped in 2026-07 merge
...
void llama_kv_cache::set_input_kq_mask(ggml_tensor * dst, const llama_ubatch * ubatch, bool causal_attn,
    ...
    // [obrain] the head-group replication and custom attention-mask overlay
    // below operate on FLOAT mask data. Upstream (post 2026-04) introduced
    // F16 mask tensors on some paths (flash attention) — the overlay is not
    // ported to F16 yet, so it only applies on the F32 path. Warn (rather
    // than silently ignore) if a custom mask was requested on an F16 mask.
    if (dst->type != GGML_TYPE_F32) {
        if (custom_mask != nullptr || (hier_mask != nullptr && !hier_mask->empty())) {
            LLAMA_LOG_WARN("%s: custom attention mask requested but the KQ mask tensor is %s (not F32) — overlay skipped\n",
