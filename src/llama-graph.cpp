...
static ggml_tensor * build_kq_mask(
    ...
    // [obrain] nhg = head-group dimension for custom per-head-group attention
    // masks (H7 hierarchical bank masks) — 1 for normal traffic (broadcast).
    // Note: the custom-mask overlay only applies on the F32 (non-FA) path,
    // see llama_kv_cache::set_input_kq_mask.
    ggml_tensor * res = ggml_new_tensor_4d(ctx, type, n_kv, n_tokens/n_stream, nhg, n_stream);
    ggml_set_input(res);
...
// [obrain] compatibility alias: upstream renamed this builder to
// build_attn_inp_kq_mask across many new call sites (DeepSeek V4 DSA, etc.);
// our build_kq_mask above is the superset (adds the optional head-groups
// dimension, defaulting to 1 = identical behaviour). Forward the plain name
// to it so upstream call sites compile unchanged.
static inline ggml_tensor * build_attn_inp_kq_mask(
        ggml_context * ctx,
        const llama_kv_cache_context * mctx,
        const llama_ubatch & ubatch,
        const llama_cparams & cparams) {
    return build_kq_mask(ctx, mctx, ubatch, cparams, 1);
}
...
void llm_graph_input_attn_kv::set_input(const llama_ubatch * ubatch) {
    ...
    // [obrain] Phase 5: fill state_kq_b tensor with external bias data (now
    // that backend has allocated memory).
